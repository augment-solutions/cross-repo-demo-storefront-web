...
export function UrlPagination({
  ...
  return (
    <nav className={cn('flex items-center justify-center gap-1', className)} aria-label="Pagination">
      {currentPage === 1 ? (
        <span className="flex h-9 w-9 items-center justify-center rounded-md border border-secondary-200 text-secondary-400 opacity-50 cursor-not-allowed">
          <ChevronLeft className="h-4 w-4" />
        </span>
      ) : (
        <Link
          href={createPageURL(currentPage - 1)}
          className="flex h-9 w-9 items-center justify-center rounded-md border border-secondary-200 text-secondary-600 transition-colors hover:bg-secondary-50"
